...
describe("CombiRouter Utilities (src/utils.ts)", () => {
  ...
  afterEach(() => {
    ...
    if (
      dom &&
      dom.window &&
      typeof dom.window.close === "function" &&
  ...
  const createMockRouter = (
    initialMatch: RouteMatch<any> | null = null,
  ): any => {
    const listeners = new Set<(match: RouteMatch<any> | null) => void>();
    ...
    const mockRouter = {
      ...
      navigate: vi.fn(async (route, params) => {
        mockRouter.isNavigating = true;
        // Notify listeners that navigation started
        for (const listener of listeners) {
          listener(mockRouter.currentMatch);
        }
        ...
        mockRouter.isNavigating = false;
        // Simulate successful navigation by updating currentMatch
        if (route && route.id) {
          mockRouter.currentMatch = {
            route,
            params: params || {},
            pathname: "/mock-path",
            search: "",
      ...
      build: vi.fn((routeToDo, paramsToBuild) => {
        ...
        let path = routeToDo.matchers
          .filter((m: any) => m.type === "path" || m.type === "param")
          .reduce((acc: any, m: any) => {
            if (m.type === "path") return acc + m.build({}); // Static path parts
            if (m.paramName && paramsToBuild[m.paramName])
              return acc + `/${paramsToBuild[m.paramName]}`;
            return acc;
          }, "");
        ...
        for (const matcher of routeToDo.matchers) {
          if (
            matcher.type === "query" &&
            matcher.paramName &&
            paramsToBuild[matcher.paramName] !== undefined
          ) {
            queryParams.push(
              `${matcher.paramName}=${encodeURIComponent(paramsToBuild[matcher.paramName])}`,
            );
          }
        }
      ...
      subscribe: vi.fn((listener) => {
        ...
        return vi.fn(() => listeners.delete(listener)); // Return an unsubscribe function
      }),
      unsubscribe: vi.fn(),
  ...
  const userRoute = route(
    path("users"),
    param("id", MockStringSchema),
    query.optional("tab", MockStringSchema),
    end,
  );
  ...
  describe("createLink()", () => {
    ...
    it("should throw if router.build returns null", () => {
      ...
      expect(() => createLink(mockRouter, homeRoute, {}, {})).toThrow(
        "[createLink] Failed to build href for route. A required parameter may be missing.",
      );
    });
  });
  ...
  describe("createActiveLink()", () => {
    it("should apply activeClassName when route is active (exact: false by default)", () => {
      const mockUserMatch: RouteMatch = {
        route: userRoute,
        params: { id: "1" },
        pathname: "/users/1",
        search: "",
      ...
      const { element } = createActiveLink(
        mockRouter,
        userRoute,
        { id: "1" },
    ...
    it("should NOT apply activeClassName if route is not active", () => {
      const mockHomeMatch: RouteMatch = {
        route: homeRoute,
        params: {},
        pathname: "/home",
        search: "",
    ...
    it("should apply activeClassName for exact match if exact is true", () => {
      ...
      const { element: parentLink } = createActiveLink(
        mockRouter,
        parentRoute,
        {},
        { activeClassName: "active-exact", exact: true },
      );
      ...
      const { element: childLink } = createActiveLink(
        mockRouter,
        childRoute,
        {},
        { activeClassName: "active-exact", exact: true },
    ...
    it("should update active class on route change", () => {
      ...
      const routeChangeListener = subscribeCall[0] as (
    ...
    it("destroy function should unsubscribe from router", () => {
      ...
      const { destroy } = createActiveLink(
        mockRouter,
        homeRoute,
        {},
        { activeClassName: "active" },
  ...
  describe("createOutlet()", () => {
    ...
    it("should clear view if no child route matches", () => {
      ...
      const parentOnlyMatch: RouteMatch = {
        route: appRoute,
        params: {},
        pathname: "/app",
        search: "",
  ...
  describe("createMatcher()", () => {
    ...
    it("should call otherwise callback if no when-condition matches", () => {
      createMatcher(mockRouter).when(homeRoute, cbHome).otherwise(cbOtherwise);

      const subscribeCall = (mockRouter.subscribe as any).mock.calls[0][0];
      const aboutMatch: RouteMatch = {
        route: aboutRoute,
        params: {},
        pathname: "/about",
        search: "",
  ...
  describe("createRouterStore()", () => {
    ...
    it("unsubscribe should remove a specific listener", () => {
      ...
      return navPromise.finally(() => {
        // Use finally to ensure checks run after nav settles
