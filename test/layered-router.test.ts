...
describe("Layered Router Architecture", () => {
  const testRoutes = [
    ...
    route(path("/user"), param("id"), meta({ name: "user" })),
  ...
  describe("Custom Layer Creation", () => {
    it("should allow creating custom analytics layer", () => {
      ...
      const withAnalytics =
        (config: { trackingId: string }) => (router: any) => ({
      ...
      const withLogging = (self: any) => {
        ...
        return {
          navigate: async (path: string, options?: any) => {
            ...
            navigationLogs.push(
              `Navigation ${result ? "successful" : "failed"}`,
            );
            return result;
          },
  ...
  describe("Layer Communication", () => {
    it("should allow layers to call methods from previous layers", () => {
      ...
      const router = createLayeredRouter(testRoutes)(
        createCoreNavigationLayer(),
      )(withPerformance())(withCaching)(withSmartPrefetch)();
  ...
  describe("Advanced Composition Patterns", () => {
    it("should support factory functions for dynamic layer creation", () => {
      const createFeatureLayer = (features: string[]) => (self: any) => {
        ...
        if (features.includes("auth")) {
          ...
          layerMethods.login = (user: any) => {
            layerMethods.currentUser = user;
          };
        }
    ...
    it("should support layer composition with makeLayered directly", () => {
      ...
      })(createCoreNavigationLayer())((self: any) => ({
        incrementCounter: () => {
  ...
  describe("Error Handling", () => {
    it("should handle layer errors gracefully", () => {
      ...
      expect(() => {
        createLayeredRouter(testRoutes)(createCoreNavigationLayer())(
          errorLayer,
        )();
  ...
  describe("Performance Characteristics", () => {
    it("should not significantly impact performance with many layers", () => {
      ...
      const router = createLayeredRouter(testRoutes)(
        createCoreNavigationLayer(),
      )(createSimpleLayer("1"))(createSimpleLayer("2"))(createSimpleLayer("3"))(
        createSimpleLayer("4"),
      )(createSimpleLayer("5"))();
...
describe("Migration and Compatibility", () => {
  ...
  it("should provide equivalent functionality to old configuration-based approach", () => {
    ...
    // New approach:
    const router = createLayeredRouter(testRoutes)(createCoreNavigationLayer())(
      withPerformance({ prefetchOnHover: true }),
    )(withScrollRestoration({ strategy: "smooth" }))();
  ...
  it("should allow gradual migration", () => {
    ...
    let router = createLayeredRouter(testRoutes)(createCoreNavigationLayer())();

    expect("prefetchRoute" in router).toBe(false);

    // Add performance later
    router = createLayeredRouter(testRoutes)(createCoreNavigationLayer())(
      withPerformance(),
    )();
...
describe("Enhanced Error Handling", () => {
  const testRoutes = [
    route(path("/"), meta({ name: "home" })),
    route(path("/valid"), meta({ name: "valid" })),
  ];
  ...
  it("should handle fallback routes properly", async () => {
    const fallbackRoute = route(path("/fallback"), meta({ name: "fallback" }));
    ...
    // Check if the route has the fallback metadata
    expect(currentMatch?.route.metadata?.name).toBe("fallback");
  });
});
...
describe("Performance Layer Validation", () => {
  const testRoutes = [
    route(path("/"), meta({ name: "home" })),
    route(path("/test"), meta({ name: "test" })),
  ];
  ...
  it("should handle lifecycle hook registration failures gracefully", () => {
    ...
    const router = createLayeredRouter(testRoutes)(withPerformance())(); // Apply performance layer without core
...
describe("Dynamic Route Management Edge Cases", () => {
  ...
  it("should handle adding routes with duplicate IDs", () => {
    ...
    const duplicateRoute = route(path("/duplicate"), meta({ name: "home" }));
  ...
  it("should handle removing non-existent routes", () => {
    ...
    const nonExistentRoute = route(path("/missing"), meta({ name: "missing" }));
