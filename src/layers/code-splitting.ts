 ...
 */
function validateCodeSplittingConfig(
  config: CodeSplittingLayerConfig,
): CodeSplittingLayerConfig {
 ...
 * ```
 */
export function createCodeSplittingLayer(
  config: CodeSplittingLayerConfig = {},
): RouterLayer<any, CodeSplittingLayerExtensions> {
  return (self) => {
    ...
    const codeSplittingConfig: CodeSplittingConfig = {
      ...
      preloadStrategy:
        validatedConfig.preloadStrategy ??
        defaultCodeSplittingConfig.preloadStrategy,
      chunkNaming:
        validatedConfig.chunkNaming ?? defaultCodeSplittingConfig.chunkNaming,
      fallback: validatedConfig.fallback ?? defaultCodeSplittingConfig.fallback,
      errorBoundary:
        validatedConfig.errorBoundary ??
        defaultCodeSplittingConfig.errorBoundary,
      preloadTimeout:
        validatedConfig.preloadTimeout ??
        defaultCodeSplittingConfig.preloadTimeout,
      retryAttempts:
        validatedConfig.retryAttempts ??
        defaultCodeSplittingConfig.retryAttempts,
      retryDelay:
        validatedConfig.retryDelay ?? defaultCodeSplittingConfig.retryDelay,
      priority: validatedConfig.priority ?? defaultCodeSplittingConfig.priority,
      connectionAware:
        validatedConfig.connectionAware ??
        defaultCodeSplittingConfig.connectionAware,
    };
    ...
    if (hasLifecycleSupport(self)) {
      try {
        ...
        self._registerLifecycleHook(
          "onRouteLoad",
          async (route: Route<any>) => {
            try {
              if (route.metadata?.lazy) {
                console.log(
                  `[CodeSplittingLayer] Loading lazy route: ${route.id}`,
                );
                await codeSplittingManager.loadChunk(String(route.id));
              }
            } catch (error) {
              console.error(
                "[CodeSplittingLayer] Error in onRouteLoad:",
                error,
              );
            }
          },
        ...
        self._registerLifecycleHook("onNavigationStart", (context: any) => {
          try {
            if (context?.to?.route?.metadata?.lazy) {
              console.log(
                `[CodeSplittingLayer] Preparing chunk for: ${context.to.route.id}`,
              );
            }
          } catch (error) {
            console.error(
              "[CodeSplittingLayer] Error in onNavigationStart:",
              error,
            );
          }
        });
        ...
        console.error(
          "[CodeSplittingLayer] Failed to register lifecycle hooks:",
          error,
        );
      }
    } else {
      console.warn(
        "[CodeSplittingLayer] Lifecycle hooks not supported by router instance",
      );
    }

    // Register a lazy route with import function
    const registerLazyRoute = (
      routeId: string,
      ...
      try {
        ...
        console.error(
          `[CodeSplittingLayer] Failed to register lazy route ${routeId}:`,
          error,
        );
      }
    };
    ...
    const preloadRoute = async (routeId: string): Promise<void> => {
      try {
        ...
        console.error(
          `[CodeSplittingLayer] Failed to preload route ${routeId}:`,
          error,
        );
      }
    };
    ...
    const getLoadingState = (routeId: string) => {
      try {
        ...
        console.error(
          `[CodeSplittingLayer] Failed to get loading state for ${routeId}:`,
          error,
        );
    ...
    const setupHoverPreloading = (
      ...
      try {
        ...
        console.error(
          `[CodeSplittingLayer] Failed to setup hover preloading for ${routeId}:`,
          error,
        );
        return () => {};
      }
    ...
    const setupVisibilityPreloading = (
      element: Element,
      routeId: string,
    ): (() => void) => {
      try {
        ...
        console.error(
          `[CodeSplittingLayer] Failed to setup visibility preloading for ${routeId}:`,
          error,
        );
        return () => {};
      }
    ...
    const updateCodeSplittingConfig = (
      newConfig: Partial<CodeSplittingLayerConfig>,
    ): void => {
      try {
        ...
        console.error(
          "[CodeSplittingLayer] Failed to update configuration:",
          error,
        );
      }
    };
    ...
    const getCodeSplittingReport = () => {
      try {
        ...
        console.error(
          "[CodeSplittingLayer] Failed to get performance report:",
          error,
        );
        return null;
      }
...
export interface CodeSplittingLayerExtensions {
  ...
  updateCodeSplittingConfig: (
    config: Partial<CodeSplittingLayerConfig>,
  ) => void;
  getCodeSplittingReport: () => any;
}

// Convenience factory with default config
export const withCodeSplitting = (config: CodeSplittingLayerConfig = {}) =>
  createCodeSplittingLayer(config);
