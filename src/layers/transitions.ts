...
import { TransitionManager, type TransitionConfig, defaultTransitionConfig } from '../features/transitions';

/**
 * Validates transitions layer configuration
 */
function validateTransitionsConfig(config: TransitionsLayerConfig): TransitionsLayerConfig {
 ...
 * ```
 */
export function createTransitionsLayer(config: TransitionsLayerConfig = {}): RouterLayer<any, TransitionsLayerExtensions> {
  return (self) => {
    ...
    const transitionConfig: TransitionConfig = {
      ...
      customTransition: validatedConfig.customTransition ?? defaultTransitionConfig.customTransition,
      skipSameRoute: validatedConfig.skipSameRoute ?? defaultTransitionConfig.skipSameRoute,
      fallbackTransition: validatedConfig.fallbackTransition ?? defaultTransitionConfig.fallbackTransition,
      debugMode: validatedConfig.debugMode ?? defaultTransitionConfig.debugMode,
      respectPreferences: validatedConfig.respectPreferences ?? defaultTransitionConfig.respectPreferences
    };
    ...
    if (hasLifecycleSupport(self)) {
      try {
        ...
        self._registerLifecycleHook('onNavigationStart', (context: NavigationContext) => {
          try {
            if (transitionConfig.enabled && context.from && context.to) {
              ...
              const transitionContext = {
                ...
                to: context.to,
                direction,
                isInitial: !context.from
              };
              
              if (transitionConfig.debugMode) {
                console.log(`[TransitionsLayer] Preparing transition: ${direction}`);
              }
              
              // Prepare transition (if needed)
              transitionManager.prepareTransition?.(transitionContext);
            }
          } catch (error) {
            console.error('[TransitionsLayer] Error in onNavigationStart:', error);
          }
        });

        // Navigation complete - execute transition
        self._registerLifecycleHook('onNavigationComplete', async (match: RouteMatch<any>, isPopState?: boolean) => {
          try {
            if (transitionConfig.enabled) {
              ...
              if (transitionConfig.debugMode) {
                console.log(`[TransitionsLayer] Executing transition: ${direction}`);
              }
              
              // Execute transition
              await transitionManager.executeTransition(transitionContext);
            }
          } catch (error) {
            console.error('[TransitionsLayer] Error in onNavigationComplete:', error);
          }
        });
        ...
        console.error('[TransitionsLayer] Failed to register lifecycle hooks:', error);
      }
    } else {
      console.warn('[TransitionsLayer] Lifecycle hooks not supported by router instance');
    }
    ...
    const executeTransition = async (context: any): Promise<void> => {
      try {
        ...
        console.error('[TransitionsLayer] Failed to execute transition:', error);
      }
    };

    // Get transition direction
    const getTransitionDirection = (from: RouteMatch<any> | null, to: RouteMatch<any>): string => {
      try {
        return transitionManager.getTransitionDirection(from, to);
      } catch (error) {
        console.error('[TransitionsLayer] Failed to get transition direction:', error);
        return 'forward';
      }
    };
    ...
    const isTransitionSupported = (): boolean => {
      try {
        return transitionManager.isSupported();
      } catch (error) {
        console.error('[TransitionsLayer] Failed to check transition support:', error);
        return false;
      }
    };

    // Update configuration
    const updateTransitionsConfig = (newConfig: Partial<TransitionsLayerConfig>): void => {
      try {
        ...
        console.error('[TransitionsLayer] Failed to update configuration:', error);
      }
    };
    ...
    const getTransitionStatus = () => {
      try {
        return {
          ...
          supportedTypes: transitionManager.getSupportedTransitions?.() || [],
          currentConfig: transitionConfig
        };
      } catch (error) {
        console.error('[TransitionsLayer] Failed to get transition status:', error);
        return { isTransitioning: false, supportedTypes: [], currentConfig: transitionConfig };
      }
    };
...
export interface TransitionsLayerExtensions {
  ...
  getTransitionDirection: (from: RouteMatch<any> | null, to: RouteMatch<any>) => string;
...
export const withTransitions = (config: TransitionsLayerConfig = {}) => createTransitionsLayer(config);
