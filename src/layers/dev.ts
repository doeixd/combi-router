 ...
 */
export function devLayer(config: DevLayerConfig = {}): RouterLayer<RouterContext, DevLayerExtensions> {
  return (router: ComposableRouter<RouterContext>) => {
    if (process.env.NODE_ENV === 'production') {
      ...
      return {
        devMode: null,
        runDevAnalysis: () => {},
        getDevReport: () => ({ error: 'Development mode disabled in production' }),
        logDevReport: () => console.log('[CombiRouter] Development mode disabled in production'),
        exportDevData: () => '{}',
        clearDevData: () => {},
        updateDevConfig: () => {}
      };
    ...
    if (config.exposeToWindow && typeof window !== 'undefined') {
      exposeDevTools(routerAdapter as any, devMode);
    }
    ...
    return {
      ...
      getDevReport: () => {
        return devMode?.getDevReport() || { error: 'Development mode not initialized' };
      },
 ...
 */
export function quickDevLayer(): RouterLayer<RouterContext, DevLayerExtensions> {
  return devLayer({
    exposeToWindow: true,
    autoAnalyze: true,
    logPerformance: true,
