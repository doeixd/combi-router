...
export class CombiRouter {
      ...
      if (this._fallbackRoute) {
        ...
        if (firstRoute) {
          this.navigate(firstRoute, {}).catch(error => {
            console.error('[CombiRouter] First route navigation failed:', error);
          });
        }
      }
    }
   ...
   * }
   */
  public async navigate<TParams>(route: Route<TParams>, params: TParams): Promise<NavigationResult> {
   ...
   */
  public async navigateSimple<TParams>(route: Route<TParams>, params: TParams): Promise<boolean> {
      ...
      try {
          ...
          if (this._transitions) {
              const transitionContext: TransitionContext = {
                  ...
                  to: newMatch,
                  direction: this._transitions.getTransitionDirection(this.currentMatch, newMatch),
          ...
          if (typeof (document as any).startViewTransition === 'function' && !this._transitions) {
          ...
          const navigationError: NavigationError = {
              type: (error as any)?.name === 'AbortError' ? 'cancelled' as NavigationErrorType : 'loader-failed' as NavigationErrorType,
  ...
  private _processParams(result: Success<{ path: object; query: any[] }>, url: URL) {
    ...
    for (const q of result.value.query) {
      ...
      const validationResult = validateSafely(q.schema, valueToParse, `Query parameter "${q.name}"`);
  ...
  private _initDevMode(): void {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        import('../dev/index').then(({ enableDevMode }) => {
  ...
  public enableDevAnalysis(): void {
    if (this._devMode) {
      ...
      console.warn('[CombiRouter] Development mode not initialized. Try calling this after router creation.');
    }
  }
  ...
  public setupHoverPrefetch(element: Element, route: Route<any>): () => void {
    ...
    if (this._codeSplitting) {
      return this._codeSplitting.setupHoverPreloading(element, String(route.id));
    }
    return () => {};
  }
   ...
   */
  public setupViewportPrefetch(element: Element, route: Route<any>): () => void {
    if (this._performance) {
      return this._performance.setupViewportPrefetch(element, String(route.id));
    }
    if (this._codeSplitting) {
      return this._codeSplitting.setupVisibilityPreloading(element, String(route.id));
    }
    return () => {};
  }
