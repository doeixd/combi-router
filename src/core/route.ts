...
export class Route<TParams = {}> {
  ...
  get isDynamic(): boolean {
    return this.matchers.some(m => m.type === 'param' || m.type === 'wildcard');
  }
  ...
  private _validateRoute(): void {
    ...
    if (duplicates.length > 0) {
      throw new (class RouteValidationError extends Error {
        ...
        { duplicates, route: this }
      );
    }

    // Check for invalid matcher combinations
    const wildcardCount = this.matchers.filter(m => m.type === 'wildcard').length;
    if (wildcardCount > 1) {
      throw new (class RouteValidationError extends Error {
        constructor(message: string, public readonly details?: any) {
          super(message);
    ...
    if (endIndex !== -1 && endIndex !== this.matchers.length - 1) {
      ...
      console.warn(`[CombiRouter] Route ${this.id} has end matcher not at the end. This route will never match.`);
    }
  }
}
...
export function extend<TBase, TExtension extends RouteMatcher[]>(
  ...
  return new Route([...baseRoute.matchers, ...additionalMatchers], { ...baseRoute.metadata }, undefined, baseRoute);
}
