...
import { createLayeredRouter, conditionalLayer } from "./layered-router";
...
import { devLayer } from "../layers/dev";
...
export class CombiRouter {
  ...
  async navigate<TParams>(
    route: Route<TParams>,
    params: TParams,
  ): Promise<NavigationResult>;
  async navigate<TParams>(
    pathOrRoute: string | Route<TParams>,
    paramsOrOptions?: TParams | any,
  ): Promise<boolean | NavigationResult> {
    if (typeof pathOrRoute === "string") {
      ...
      if (url === null) {
        return {
          ...
          error: {
            ...
            message:
              "Failed to build URL. A required parameter may be missing.",
            route: pathOrRoute,
      ...
      // Create promise and its resolver for the controller
      let resolvePromise: (result: NavigationResult) => void;
      const navigationPromise = new Promise<NavigationResult>((resolve) => {
        resolvePromise = resolve;
      });

      // Create navigation controller for compatibility
      ...
      try {
        ...
        const result = {
          ...
          error: success
            ? undefined
            : { type: "navigation-failed", message: "Navigation failed" },
  ...
  peek(route: Route<any>, params: Record<string, any>): Promise<void> {
    if (
      "peek" in this._layeredRouter &&
      typeof this._layeredRouter.peek === "function"
    ) {
      return this._layeredRouter.peek(route, params);
    }
    console.warn(
      "[CombiRouter] peek() not supported in this router configuration",
    );
  ...
  addRoute(route: Route<any>): boolean {
    if (
      "addRoute" in this._layeredRouter &&
      typeof this._layeredRouter.addRoute === "function"
    ) {
      return this._layeredRouter.addRoute(route);
    }
    console.warn(
      "[CombiRouter] Dynamic route addition not supported in this router configuration",
    );
    return false;
  }
  ...
  removeRoute(route: Route<any>): boolean {
    if (
      "removeRoute" in this._layeredRouter &&
      typeof this._layeredRouter.removeRoute === "function"
    ) {
      return this._layeredRouter.removeRoute(route);
    }
    console.warn(
      "[CombiRouter] Dynamic route removal not supported in this router configuration",
    );
    return false;
  }
