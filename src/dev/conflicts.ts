...
export class RouteConflictDetector {
    ...
    staticPaths.forEach((routes, path) => {
      if (routes.length > 1) {
        conflicts.push({
          ...
          conflictPath: path,
          severity: 'error',
          suggestion: 'Multiple routes have identical static paths. Ensure each route has a unique path pattern.',
          details: {
  ...
  private detectParameterConflicts(): RouteConflict[] {
    ...
    pathSegments.forEach((segmentGroup, basePath) => {
      const staticRoutes = segmentGroup.filter(route => !this.hasParameterAtLevel(route, basePath.split('/').length));
      const paramRoutes = segmentGroup.filter(route => this.hasParameterAtLevel(route, basePath.split('/').length));
  ...
  private detectWildcardConflicts(): RouteConflict[] {
    ...
    wildcardRoutes.forEach(wildcardRoute => {
      ...
      const affectedRoutes = this.routes.filter(route => 
        route !== wildcardRoute && this.isOvershadowedByWildcard(route, wildcardRoute)
      );
    ...
    potentialMatches.forEach(group => {
      if (group.length > 1) {
        conflicts.push({
          ...
          severity: 'warning',
          suggestion: 'Multiple routes could match similar URL patterns. Consider adding more specific constraints or using different path structures.',
          details: {
    ...
    paramGroups.forEach((routes, paramKey) => {
      ...
      if (typeConflicts.length > 0) {
        conflicts.push({
          ...
          severity: 'warning',
          suggestion: 'Routes have parameters at the same position with different validation schemas. This may cause unexpected behavior.',
          details: {
            parameterPosition: paramKey,
            schemas: typeConflicts.map(route => this.getParameterSchemas(route))
          }
        });
      }
    });
    ...
    for (const r of route.routeChain) {
      for (const matcher of r.matchers) {
        if ((matcher as any).type === 'path') {
          currentLevel++;
        } else if ((matcher as any).type === 'param' && currentLevel === level) {
          return true;
        }
      }
    }
  ...
  private isOvershadowedByWildcard(route: Route<any>, wildcardRoute: Route<any>): boolean {
  ...
  private doesRouteBlock(earlierRoute: Route<any>, laterRoute: Route<any>): boolean {
    // Check if the earlier route would match URLs intended for the later route
    const earlierPattern = this.normalizePattern(this.getRoutePattern(earlierRoute));
    const laterPattern = this.normalizePattern(this.getRoutePattern(laterRoute));
  ...
  private patternCovers(generalPattern: string, specificPattern: string): boolean {
    ...
    for (let i = 0; i < generalParts.length; i++) {
      ...
      if (generalPart !== specificPart && generalPart !== ':param') return false;
    }
  ...
  private findPotentiallyAmbiguousGroups(): Route<any>[][] {
    ...
    this.routes.forEach(route => {
      ...
      const similarRoutes = this.routes.filter(other => 
        other !== route && 
  ...
  private findCommonPath(routes: Route<any>[]): string {
    ...
    const paths = routes.map(r => r.staticPath.split('/').filter(p => p));
  ...
  private generateSummary(conflicts: RouteConflict[], errors: number, warnings: number): string {
...
export function logRouteConflicts(routes: Route<any>[]): void {
  ...
  if (analysis.conflicts.length > 0) {
    analysis.conflicts.forEach(conflict => {
      const prefix = conflict.severity === 'error' ? '❌' : 
                    conflict.severity === 'warning' ? '⚠️' : 'ℹ️';
      
      console.group(`${prefix} ${conflict.type.toUpperCase()}: ${conflict.conflictPath}`);
      console.log('Routes:', conflict.routes.map(r => r.id));
 ...
 */
export function findConflicts(routes: Route<any>[], type: ConflictType): RouteConflict[] {
  const analysis = analyzeRouteConflicts(routes);
