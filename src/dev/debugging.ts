...
export function inspectRoute(route: Route<any>): string {
  ...
  if (route.metadata.guards?.length) parts.push(`- Guards: ${route.metadata.guards.length}`);
  if (route.metadata.cache) parts.push('- Has cache config');
  if (route.metadata.lazy) parts.push('- Lazy loaded');
  
  parts.push(`- Matchers: ${route.matchers.map((m: any) => m.type).join(' → ')}`);
...
export function inspectMatch(match: RouteMatch<any>): string {
  ...
  if (match.data) {
    parts.push(`- Data: ${typeof match.data === 'object' ? JSON.stringify(match.data, null, 2) : match.data}`);
  }
...
export function debugRouter(router: CombiRouter): void {
  ...
  console.log('Current match:', router.currentMatch ? inspectMatch(router.currentMatch) : 'None');
...
export function monitorNavigation(router: CombiRouter): () => void {
  ...
  const unsubscribe = router.subscribe((match) => {
    if (router.isNavigating && !startTime) {
      ...
      console.log(`[CombiRouter] Navigation completed in ${duration.toFixed(2)}ms`);
      if (match) {
...
export class RouterDebugger {
  ...
  public analyzeRoutes(): RouteAnalysis[] {
    return this.router.routes.map(route => this.analyzeRoute(route));
  }
  ...
  public validateRouteConfiguration(): ValidationReport {
    ...
    this.router.routes.forEach(route => {
      ...
      if (!routeReport.valid) {
        report.valid = false;
        report.errors.push(...routeReport.issues.filter(i => i.includes('Error:')));
        report.warnings.push(...routeReport.issues.filter(i => i.includes('Warning:')));
      }
    });
  ...
  private validateSingleRoute(route: Route<any>): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  private validateGlobalConfiguration(report: ValidationReport): void {
    ...
    patterns.forEach((routes, pattern) => {
      if (routes.length > 1) {
        report.errors.push(`Error: Multiple routes with same pattern: ${pattern}`);
        report.valid = false;
      }
    });
  }
  ...
  public generateRouteTree(): string {
    return visualizeRouteTree(this.router.routes);
  }
...
export function analyzeRouterStructure(router: CombiRouter): void {
  ...
  if (complexRoutes.length > 0) {
    console.warn('High complexity routes:', complexRoutes.map(a => a.route.id));
  }
