...
export class DevWarningSystem {
  ...
  constructor(router: CombiRouter, config: Partial<WarningConfig> = {}) {
    ...
    this.config = {
      ...
      severityFilter: [WarningSeverity.INFO, WarningSeverity.WARNING, WarningSeverity.ERROR],
  ...
  private checkUnusedParameters(): void {
    ...
    routes.forEach(route => {
      ...
      const hasGuards = route.metadata.guards && route.metadata.guards.length > 0;
      ...
      if (paramNames.length > 0 && !hasLoader && !hasGuards) {
        this.addWarning({
          ...
          route,
          suggestion: 'Consider adding a loader to fetch data using these parameters, or remove unused parameters'
        });
      }
    });
  }
    ...
    patterns.forEach((routeGroup, pattern) => {
      if (routeGroup.length > 1) {
        this.addWarning({
          ...
          suggestion: 'Ensure route patterns are unique or use proper route ordering',
  ...
  private checkParameterConflicts(routes: Route<any>[]): void {
    const pathGroups = new Map<string, { static: Route<any>[], dynamic: Route<any>[] }>();
    ...
    pathGroups.forEach((group, basePath) => {
      if (group.static.length > 0 && group.dynamic.length > 0) {
        this.addWarning({
          ...
          suggestion: 'Ensure static routes are defined before dynamic ones in your route array',
          details: {
    ...
    routes.forEach(route => {
      ...
      if (route.depth > 5) {
        this.addWarning({
          ...
          route,
          suggestion: 'Consider flattening the route hierarchy or breaking into smaller sub-routers'
        });
      }
      ...
      if (route.paramNames.length > 4) {
        this.addWarning({
          ...
          route,
          suggestion: 'Consider using fewer parameters or query parameters for optional data'
        });
      }
    });
  }
    ...
    routes.forEach(route => {
      ...
      if (hasWildcardAncestor) {
        this.addWarning({
          ...
          severity: WarningSeverity.WARNING,
          message: 'Route may be unreachable due to wildcard matcher in ancestor route',
          route,
          suggestion: 'Ensure wildcard routes are ordered correctly and specific routes come first'
        });
      }
    });
  }
    ...
    routes.forEach(route => {
      ...
      if (route.metadata.guards && route.metadata.guards.length > 3) {
        this.addWarning({
          ...
          route,
          suggestion: 'Consider combining guards or using more efficient guard logic'
        });
      }
    });
  }
      ...
      setTimeout(() => {
        ...
        if (duration > 5000) { // 5 second threshold
          this.addWarning({
            ...
            route: match.route,
            suggestion: 'Consider optimizing data fetching or adding loading states'
          });
        }
      }, 0);
    }
 ...
 */
export function createWarningSystem(router: CombiRouter, config?: Partial<WarningConfig>): DevWarningSystem | null {
