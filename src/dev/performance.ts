...
export class PerformanceMonitor {
  ...
  public endNavigationTiming(_match: RouteMatch<any> | null): void {
    if (!this.currentNavigation) return;

    const endTime = performance.now();
    this.currentNavigation.endTime = endTime;
    this.currentNavigation.duration =
      endTime - this.currentNavigation.startTime;
    this.currentNavigation.cancelled =
      this.router.currentNavigation?.cancelled || false;
  ...
  public endLoaderTiming(
    timingId: string,
    result: { cached?: boolean; dataSize?: number; error?: any } = {},
  ): void {
  ...
  private analyzeLoaderPerformance(timing: LoaderTiming): void {
    ...
    if (timing.dataSize && timing.dataSize > 1024 * 1024) {
      // 1MB
      console.warn(
  ...
  public endGuardTiming(
    ...
    this.markPerformance(
      `guard-end-${timing.routeId}-${timing.guardIndex}-${timingId}`,
    );
  ...
  private analyzeNavigationPerformance(timing: NavigationTiming): void {
    if (
      timing.duration &&
      timing.duration > this.config.slowNavigationThreshold
    ) {
      console.warn(
      ...
      Object.entries(timing.phases).forEach(([phase, duration]) => {
        if (duration > timing.duration! * 0.3) {
          // Phase taking >30% of total time
          console.warn(
            `  - ${phase} phase took ${duration.toFixed(2)}ms (${((duration / timing.duration!) * 100).toFixed(1)}%)`,
          );
        }
      });
    }
  ...
  private getNavigationMetrics() {
    ...
    return {
      ...
      averageDuration,
      slowestNavigation:
        successful.sort((a, b) => (b.duration || 0) - (a.duration || 0))[0] ||
        null,
      fastestNavigation:
        successful.sort((a, b) => (a.duration || 0) - (b.duration || 0))[0] ||
        null,
    };
  }
  ...
  private getLoaderMetrics() {
    ...
    const totalDataSize = loaders.reduce(
      (total, l) => total + (l.dataSize || 0),
      0,
    );
  ...
  private getGuardMetrics() {
    ...
    return {
      ...
      averageDuration,
      slowestGuard:
        guards.sort((a, b) => (b.duration || 0) - (a.duration || 0))[0] || null,
    };
  }
  ...
  public getInsights(): PerformanceInsights {
    ...
    if (
      metrics.navigation.averageDuration > this.config.slowNavigationThreshold
    ) {
      bottlenecks.push("Slow average navigation time");
      recommendations.push(
        "Consider optimizing loaders or adding loading states",
      );
    }
    ...
    const cacheHitRate =
      metrics.loaders.total > 0
        ? metrics.loaders.cacheHits / metrics.loaders.total
        : 0;
    if (cacheHitRate < 0.3) {
      recommendations.push(
    ...
    const blockRate =
      metrics.guards.total > 0
        ? metrics.guards.blocked / metrics.guards.total
        : 0;
    if (blockRate > 0.2) {
      warnings.push(
        "High guard block rate - users may be frequently blocked from routes",
      );
    }
    ...
    if (metrics.navigation.failed > metrics.navigation.successful * 0.1)
      score -= 20;
  ...
  public logReport(): void {
    ...
    console.log(
      `Success Rate: ${((metrics.navigation.successful / metrics.navigation.total) * 100).toFixed(1)}%`,
    );
    console.log(
      `Average Duration: ${metrics.navigation.averageDuration.toFixed(2)}ms`,
    );
    console.groupEnd();
    ...
    console.log(
      `Cache Hit Rate: ${((metrics.loaders.cacheHits / metrics.loaders.total) * 100).toFixed(1)}%`,
    );
    console.log(
      `Average Duration: ${metrics.loaders.averageDuration.toFixed(2)}ms`,
    );
    console.groupEnd();
    ...
    console.log(
      `Block Rate: ${((metrics.guards.blocked / metrics.guards.total) * 100).toFixed(1)}%`,
    );
    console.log(
      `Average Duration: ${metrics.guards.averageDuration.toFixed(2)}ms`,
    );
    console.groupEnd();
